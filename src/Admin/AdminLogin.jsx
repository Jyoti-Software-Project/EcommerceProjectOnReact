...
const validate =(values) =>{
    ...
    if(!values.name){
        ...
        document.getElementById("validName").style.fill="#FF0000";
    }
    if(values.name.length<6){
        errors.name = "*Minimum six character needed*";
        document.getElementById("validName").style.fill="#FF0000";
    }
    else{
        document.getElementById("validName").style.fill="#008000";
    }
...
const AdminLogin = () => {
    ...
    const data=["system", "Root@123"];

    /* Value send to local storage */
    const navigate = useNavigate();
    const handleLogin=()=>{
        if(formik.values.name===data[0] && formik.values.password===data[1]){
    ...
    return (
        <div className="container-fluid px-0 py-5 myAdminLoginContainer user-select-none" style={{minHeight: "27rem"}}>
            <form onSubmit={formik.handleSubmit} onReset={formik.handleReset}>
                <table className="container bg-secondary-subtle rounded-4 mySecondAdminLoginContainer">
                    <thead>
                        <tr>
                            <th colSpan="3" className="text-center pb-3 pt-5">
                                <div className="position-relative z-3 rounded-circle mx-auto" style={{width: "9rem", boxShadow: "0 0 20px #FFFFFF"}}>
                                    <svg className="position-relative top-0 myAdminLoginIcon" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 640 512" fill="rgba(246, 254, 2, 1)" height="75" width="125" transform="scale(-1, 1)">
                                    ...
                                    <span className="fs-4 fw-semibold fst-italic myAdminLoginName position-absolute">Afistracate</span>
                                </div>
                            </th>
                        </tr>
                    </thead>
                    <tbody>
                        ...
                        <tr>
                            <td className="ps-5 py-4"><label htmlFor="staticName" className="col-sm-2 col-form-label fs-5 fw-semibold text-light">Name<span className="text-light">*</span></label></td>
                            ...
                            <td className="pe-5 py-4">
                                <div className="input-group col-sm-10">
                                    ...
                                    <input type="text" className="form-control fs-6 fw-semibold" id="staticName" name="name" value={formik.values.name} onInput={formik.handleChange} aria-label="name" autoFocus />
                                    <span className="input-group-text rounded bg-light">
                                        <svg xmlns="http://www.w3.org/2000/svg" width="22" height="22" fill="#FF0000" id="validName" className="bi bi-check-square-fill px-0" viewBox="0 0 16 16">
                                            <path d="M2 0a2 2 0 0 0-2 2v12a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V2a2 2 0 0 0-2-2H2zm10.03 4.97a.75.75 0 0 1 .011 1.05l-3.992 4.99a.75.75 0 0 1-1.08.02L4.324 8.384a.75.75 0 1 1 1.06-1.06l2.094 2.093 3.473-4.425a.75.75 0 0 1 1.08-.022z"/>
                                        </svg>
                                    </span>
                                </div>
                                <span className="text-danger">{formik.errors.name ? <em>{formik.errors.name}</em> : null}</span>
                            </td>
                        </tr>
                        <tr>
                            ...
                            <td className="pe-5 py-4">
                                ...
                                <span className="text-danger">{formik.errors.password ? <em>{formik.errors.password}</em> : null}</span>
                            </td>
                        </tr>
                        <tr>
                        ...
                        <tr>
                            <th colSpan="3" className="text-center pt-3 pb-5">
                                <label htmlFor="AdminRegistrationPage" className="fs-5 fw-semibold text-light text-capitalize">Don't&nbsp;Have&nbsp;an&nbsp;account&nbsp;?&nbsp;</label>
                                <NavLink to="/Admin/AdminRegister" className="fs-5 fw-semibold text-decoration-none myAdminRegisterLink" id="AdminRegistrationPage">Register</NavLink>
                            </th>
                        </tr>
                    </tbody>
                </table>
